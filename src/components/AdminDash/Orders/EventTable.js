...
import EVT_ORDERS from '../../../graphQl/queries/protected/eventOrders';
...
const EventTable = ({ metaData }) => {
  ...
  const { data, loading, fetchMore, refetch } = useQuery(EVT_ORDERS, {
    variables: {
      limit,
      ...debouncedOptions,
    ...
    fetchMore({
      ...
      updateQuery: (prev, { fetchMoreResult }) => {
        ...
        return {
          eventOrders: {
            ...prev.eventOrders,
            orders: prev.eventOrders.orders.concat(fetchMoreResult.eventOrders.orders),
          },
        };
      },
    });
  ...
  useEffect(() => {
    if (data) setPage(Math.floor(data.eventOrders.orders.length / limit));
  }, [limit, data]);
  ...
  const { orders, total } = data.eventOrders;
  ...
  return (
    <>
      <h2 className={styles.usrHeading}>{`EVENT ORDERS (TOTAL:${total})`}</h2>
      <TableWrapper options={options} handleChange={handleChange}>
        {orderItems}
        {orders.length < total && (
          <Button
      ...
      <OrderModal
        ...
        canEditOrders={metaData.canEditOrders}
        orderType="evt"
      />
    </>
  );
