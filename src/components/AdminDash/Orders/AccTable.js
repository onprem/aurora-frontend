...
import ACC_ORDERS from '../../../graphQl/queries/protected/accOrders';
...
const AccTable = ({ metaData }) => {
  ...
  const { data, loading, fetchMore, refetch } = useQuery(ACC_ORDERS, {
    variables: {
      limit,
      ...debouncedOptions,
    ...
    fetchMore({
      ...
      updateQuery: (prev, { fetchMoreResult }) => {
        ...
        return {
          accOrders: {
            ...prev.accOrders,
            orders: prev.accOrders.orders.concat(fetchMoreResult.accOrders.orders),
          },
        };
      },
    });
  ...
  useEffect(() => {
    if (data) setPage(Math.floor(data.accOrders.orders.length / limit));
  }, [limit, data]);
  ...
  const { orders, total } = data.accOrders;
  ...
  return (
    <>
      <h2 className={styles.usrHeading}>{`ACCO ORDERS (TOTAL:${total})`}</h2>
      <TableWrapper options={options} handleChange={handleChange}>
        {orderItems}
        {orders.length < total && (
          <Button
      ...
      <OrderModal
        ...
        canEditOrders={metaData.canEditAcc}
        orderType="acc"
      />
    </>
  );
